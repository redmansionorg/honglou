...
export default function Debug() {
  ...
  const createChaptersForNovel = async (novelId, novelTitle) => {
    try {
      ...
      for (const chapter of chapterData) {
        console.log('创建章节时的内容：', chapter.content);
        console.log('内容中的换行符数量：', (chapter.content.match(/\n/g) || []).length);
        const createdChapter = await Chapter.create(chapter);
        console.log('创建后的章节ID：', createdChapter.id);
      }
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 space-y-8">
      <h1 className="text-3xl font-bold text-center">数据调试页面</h1>
      
      <Card>
        <CardHeader>
          <CardTitle>小说列表 ({novels.length})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {novels.map((novel) => (
            <div key={novel.id} className="border p-4 rounded-lg space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold">{novel.title}</h3>
                  <p className="text-sm text-gray-600">ID: {novel.id}</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>章节列表 ({chapters.length})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {chapters.map((chapter) => (
            <div key={chapter.id} className="border p-2 rounded text-sm">
              <p><strong>章节:</strong> {chapter.title}</p>
              <p><strong>小说ID:</strong> {chapter.novel_id}</p>
              <p><strong>章节号:</strong> {chapter.chapter_number}</p>
            </div>
          ))}
        </CardContent>
      </Card>
